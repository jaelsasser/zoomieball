...
- [ ] **M0 — make the permanent CPU path conforming.**
  ...
  - Normative anchor: the normative tick order and fixed arithmetic in `GAME_TICK.md`, plus the CPU reference contract in `DESIGN.md`.
  - Boundary test: one public tracer crosses graph selection, perception, same-tick coach mailboxes, body inference, two physics steps, rewards, layered witnesses, and presentation without bypassing the controller.
...
- [ ] **M1 — ship the permanent CPU/Canvas2D compatibility tier.**
  ...
  - Boundary test: a 10v10 match runs CPU physics, perception, Zoomie inference/learning, HUD, labels, and one packed snapshot publication at real time.
  - Completion command: `cargo test -p zoomieball-web canvas2d_10v10_realtime`.

- [ ] **M2a — bring up GPU physics against the CPU shadow.**
  - Prerequisite: M0 physics witnesses and stage-level CPU fixtures are stable.
...
- [ ] **M5 — assemble the WebGPU application shell.**
  - Prerequisite: M1 fallback and M2b/M3 primary paths pass their parity and residency gates.
  - Normative anchor: tier selection, HUD, persistence, and profiling contracts in `DESIGN.md` and `docs/architecture.md`.
  - Boundary test: device profiling selects WebGPU or Canvas2D deterministically, import/export preserves v0 local state, and both tiers expose the same match/HUD semantics.
...
- [ ] **Acknowledge graph triggers and verb/target shapes.**
  - Prerequisite: the extended CPU graph-v0 proposal has concrete checked-in play examples.
